...
\begin{align*}
  \frac{\xx_i(t + \Delta t) -  \xx_i(t)}{dt} = \uu_i(t+\Delta t) +
    \sum_{j \neq i} \uu_j(t).
\end{align*}
...
streamlines. The effect of the contact force on the streamlines can be seen in Figure \ref{fig:shear_cross}. Keeping the minimum separation fixed at $\delta = 3h$, we vary the initial $y$ coordinate of the left particle. Initially the particle the blue streamline is on the bottom, but after the repulsion force is applied it is now on top. The streamlines cross at around $x = -0.25$. 

\begin{figure}[!h]
\begin{center}
\includegraphics{figures/shear_streamlines_cross.pdf}
\end{center}
\caption{Repulsion forces causes streamlines to cross. Keeping the minimum separation fixed, we vary the starting $y$ location of the left particle.}\label{fig:shear_cross}
\end{figure}
