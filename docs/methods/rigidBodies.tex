...
needs to be solved with the inequality constraint that the STIV is greater
than zero.  The definition of the STIV includes a parameter that guarantees
...
inequality constraint that the STIV is greater than or equal to zero.  Therefore, the
...
\todo{The STIV must be greater than or equal to 0 (in fact it is identically 0 for non-intersecting bodies)}

The resulting equation is a non-linear complementary problem (NCP)
To solve the NCP, the problem is linearized as a sequence of linear
complementary problems (LCPs) that are solved until the STIV is
greater than or equal to zero.  For small minimum separation distances, it is possible
...
contact, then the forces returned by the STIV are not
guaranteed to sum to zero.  We improve the validity of the method by
always requiring that the suspension's total force added to the system is zero,
meaning
\begin{align*}
  \sum_{j=1}^{M_p} \FF^\gamma_j = \mathbf{0}. 
\end{align*}
\todo{net torque can be nonzero, conservation of torque is not necessary}
...
\end{align*}
A low cost first-order time stepper can be justified since we are
...
stepping method is focus of the work of Lu et al.~\cite{Lu2017}.   There,
...
integration~\cite{sid-sir1988}\todo{this reference is not in my bibliography}.  The result is a spectrally accurate
