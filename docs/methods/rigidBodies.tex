...
energy dissipation.  This expressions are standard~\cite{Power1993}, but we summarize
...
discuss a numerical methods for related to the collision algorithm
(Section~\ref{sec:contact}), and computing the pressure and stress
...
needs to be solved with the inequality constraint that the STIV is less
than 0.  The definition of the STIV includes a parameter that guarantees
that not only do bodies intersect, but they maintain minimum separation
distance.  Following~\cite{Lu2017}, we use a Lagrange multiplier,
$\lambda$, to satisfy the non-positive STIV inequality constraint.  The
resulting equations are  the incompressible Stokes equations with body
forces and torques that depend on the gradient of the STIV, and the
inequality constraint that the STIV is non-positive.  Therefore, the
governing equations are similar to~\eqref{eqn:BIEformulation}, except
that the force- and torque-free conditions have been changed.

The resulting equation is a non-linear complementary problem (NCP)
Rather than solving the NCP, the problem is linearized as a linearly
complementary problem (LCP) that is solved iteratively until the STIV is
less than zero.  For small minimum separation distances, it is possible
that the LCP converges very slowly, or not at all, to the solution of
the NCP.  In Section~\ref{sec:temporal}, we describe two methods to
avoid this slow convergence.
...
contact, then the forces and torques returned by the STIV are not
guaranteed to sum to zero.  We improve the validity of the method by
always requiring that the suspension's total force and torque are zero,
meaning
\begin{align*}
  \sum_{j=1}^{M_p} \FF^\gamma_j = \mathbf{0}, \qquad 
  \sum_{j=1}^{M_p} L^\gamma_j = 0.
\end{align*}
...
\todo[inline]{Why not just make all the torques also add to 0?}
...
\end{align*}
We justify using a low cost first-order time stepper since we are
interested in statistical, rheological, an bulk quantities rather than
the individual trajectories of the rigid bodies.  If high-order accuracy
is desired, a Runge-Kutta or deferred correction
method~\cite{Quaife2015, qua-bir2016} can be applied.
...
requires many iterations to solve the NCP, this indicates that the timne
...
taken.  Therefore, if the STIV at each LCP iteration is not
decreasing sufficiently fast, we abort the time step and restart with
the time step size $\Delta t/2$. Conversely, if the STIV decreases
quickly at each LCP iteration, then, at the next time step, we increase
...
iterations, and the time step size is reduced.  In contrast, if the
interaction between the bodies is discretized implicitly, then the
stiffness caused by the nearly-touching bodies is resolved and a larger
time step can be taken.

The leading source of stiffness for a particular rigid body is the
velocity field induced by the body itself. This leading source of
stiffness is controlled with a {\em locally implicit} discretization
\begin{align}
...
stepping method is focus of the work of Lu et al.~\cite{Lu2017}.   Then,
to avoid stiffness, caused by nearly-touching bodies, a sufficiently
large minimum separation distance is used.
...
integration~\cite{sid-sir1988}.  The result is a spectrally accurate
...
In many industrial applications, for example pulp and paper manufacturing, suspensions of rigid ellongated fibers are encountered. Motivated by these suspensions we investigate rheological and statistical
...
$\lambda$ of 1, 3, and 6.  In all the examples, $nu M 1/ell^2$, so all the
...
\todo[inline]{The conclusions still need some work ... running out of
steam}
We have developed a stable time stepping method that is capable of
simulating dense suspensions of rigid bodies without introducing
stiffness or requiring a large minimum separation distance.  Repulsion
forces between bodies are added that are guaranteed to be sufficiently
large enough to avoid overlap between bodies. Using this new time
stepper we are able to simulate concentrated suspensions while
maintaining a reasonably large time step. This allows us to compute
rheological parameters such as the orientation of rigid fibers in a
shear flow and the effective shear viscosity of a suspension. 
...
suspensions have been developed, see for
example~\cite{cor-gre-rac-vee2017}.  The computation of the STIV in
three-dimensions has also been developed~\cite{Harmon2011} but it has
not yet been applied to suspensions.
