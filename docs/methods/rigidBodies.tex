...
\todo[inline]{Bryan hasn't edited section 3}
...
(Section~\ref{sec:temporal}). Each time step requires solving a block
diagonal preconditioned dense linear system that is solved with GMRES
and accelerated with the fast multipole method~\cite{Greenbaum1992}
(Section~\ref{sec:fast}).  We also discuss a numerical methods for
related to the collision algorithm (Section~\ref{sec:contact}), and computing the
pressure and stress (Section~\ref{sec:pressure_stres}).
...
Motivated by applications in \todo[inline]{something simple that can be
tied to slender body suspensions}, we investigate rheological and statistical
...
$\psi$ of 1, 3, and 6.  In all the examples, $nu M 1/ell^2$, so all the
...
\todo[inline]{If using $\psi$ for the aspect ratio, then some of the
figures need fixed.  Make sure to get them all.  Sachin seems to have
used $\lambda$. I don't care what we use, but we have to use one thing}
...
varying volume fraction $\phi$ and aspect ratio $\psi$. From left to
right: 1) $\phi=5\%$, $\psi= 3$, 2) $\phi=5\%$, $\psi=6$, 3)
$\phi=10\%$, $\psi=3$, 4) $\phi=10\%$,
...
flow, where the body will align with the shear direction \emph{on
average}. However, at any point in time it will in general not be
aligned with the shear direction \todo[inline]{not sure what you mean
here}. Bodies with a high aspect ratio rotate quickly when then they are
...
bodies with aspect ratio $\psi=6$. This is because higher aspect ratio
...
in~\cite{Lu2017}.
\todo[inline]{need to say something about the smoothing you've done in
the figure.  This isn't discussed anywhere in the body}
\begin{figure}[!h]
\begin{center}
...
\begin{center}
\begin{tabular}{c| c | c |  c}
$\psi$ & 5\% area fraction  & 10\% area fraction  & 15\% area fraction\\
\hline
1 & 1.12 & 1.22 & 1.42 \\
3 & 1.10 & 1.23 & 1.36 \\
6 & 1.08 & 1.18 & 
