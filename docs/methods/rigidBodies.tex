...
\todo[inline]{We use fiber in the introduction which I am fine with
since that's the application.  After that, we need to be careful. Are we
simulating rigid bodies, bodies, particles, or rigid particles?  For
now, I've changed everything to bodies and rigid bodies.  I am open to
discussion, though}

\todo[inline]{That's fine with me. -LB}
...
rotation is hindered; however, it is found that the statistical properties
...
rigid bodies often come into contact or even overlap. \todo[inline]{Contact is not allowed using Stokes equations, but it's not necessarily unphysical; overlap between particles is unphysical.}Therefore, we apply
a contact algorithm that allows rigid particles to come very close to
one another, but guarantees that contact is
...
dimension). \todo{I cited Dens' talk from BIRS in the conclusion where he does STIV in 3D for vesicles. It's also been done in papers (but not for suspensions)}
...
variety of adaptive time stepping methods, and we consider two.\todo{It makes it sound like we actually implemented two methods in this paper} First,
