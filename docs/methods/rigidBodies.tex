...
(\nabla \mathbf{v} + \nabla \mathbf{v}^\intercal)/2$. Assuming purely
...
Particular challenges with the BIE formulation, especially for dense suspensions, are large numbers of required GMRES iterations, expensive matrix-vector multiplication, and weakly-singular and nearly-singular
integrands.  We develop a second-kind boundary integral equation, so the
number of required GMRES iterations is
mesh-independent~\cite{Campbell1996}, but the number of iterations is
not geometry-independent.  We are able to use an exact block-diagonal
preconditioner since the number of unknowns per particle is moderate in
two dimensions.  However, to extend our work to three dimensions, other
preconditioners such as the IFMM~\cite{qua-cou-dar2018, cou-pou-dar2017}
multigrid~\cite{Quaife2015, bra-lub1990, hem-sch1981}, or sparse
approximate inverses~\cite{che2000} will have to be developed.  While
the required number of GMRES iterations is important, the cost is
dominated by the cost of applying dense matrix-vector multiplication.
We use the fast multipole method~\cite{Greengard1987, gre-gre-may1992},
but other fast summation methods are possible~\cite{bar-hut1986,
kli-tor2014}.  As an alternative to using an iterative solver, direct
solvers can be used to solve BIEs~\cite{mar-bar-gil-vee2016}, but these
solvers would have to be updated at each time step since the geometry is
moving.  Finally, we avoid weakly-singular integrands by using a
double-layer potential, but nearly-singular integrands are unavoidable
in dense suspensions.  We apply a interpolation-based quadrature
method~\cite{Ying2006, Quaife2014} since it is efficient and extends to
three dimensions, but other near-singular integration schemes are
possible~\cite{Klockner2013, Barnett2015, Beale2016, Helsing2008,
Kropinski1999, Mammoli2006}.


%Particle suspensions in a viscous fluid is a
%well studied discipline.  The texts~\cite{Pozrikidis1992, Guazzelli2011}
%provide an excellent overview of the theoretical formulation and
%applications of microhydrodynamics, while~\cite{Karrila1991} places the
%theory in the context of linear operators and provides some details on
%the numerical methods needed. This paper draws largely on work done for
%2D vesicle suspensions~\cite{Quaife2014, Quaife2015, Quaife2015a,
%Rahimian2010, Lu2017}. In particular we adopt the completed double layer
%potential formulation~\cite{Power1987, Power1993, Karrila1989} that
%leads to a well conditioned linear system. The resulting equations can
%be discretized using the Galkerkin based boundary element
%method~\cite{Mammoli1999, Mammoli2002, Mammoli2006} however we will the
%spectrally accurate collocation Nystr\"{o}m method.  
%The kernels arising
%from the integral formulation are are weakly singular as target points
%approach the boundary, so specialized integration techniques are needed.
%Upsampling~\cite{Kropinski1999, Mammoli2006} is one possible avenue,
%however for dense suspensions particles can become arbitrarily close and
%the memory requirements needed to accurately evaluate such closely
%touching particles can become prohibitively expensive. Quadrature by
%expansion~\cite{Klockner2013} uses local expansions to evaluate the
%kernels to high accuracy near the particle surface and has been used to
%solve the resistance problem in Stokes flow~\cite{kli-tor2016}.An
%interpolation and limited upsampling based approach~\cite{Ying2006,
%Quaife2014} is simpler to implement and gives high-order accuracy even
%near the boundary. 
...
discretization
\begin{align*}
  \DD[\eeta](\xx) \approx \DD^{n}[\eeta^]
\end{align*}


A secondary
source of stiffness is caused by nearly touching bodies.  The amount of
stiffness is a direct consequence of how the double-layer potential
in~\eqref{eqn:BIEformulation} is discretized. 
...
\begin{itemize}
  ...
  \item {\bf Taylor-Couette Flow}: With the ability to do higher volume fraction suspensions
...
force is used to enforce the minimum separation. Here the number of particles per unit volume $\nu$ is approximately 3.1. This is greater than $1/L^2 = 1.108$, which puts the simulation well outside the dilute regime. }\label{fig:taylor_green}
\end{figure}
...
\cite{Lu2017}. The time averaged bulk shear stresses for the the plots in Figure \ref{fig:torque} are given in Table \ref{tab:viscosity}.
\begin{figure}[!h]
\begin{center}
\includegraphics{figures/couette_torque_005.pdf}\\
\includegraphics{figures/couette_torque_010.pdf}\\
\end{center}
...
\begin{center}
\begin{tabular}{c| c | c |  c}
$\psi$ & 5\% volume fraction  & 10\% volume fraction  & 15\% volume fraction\\
\hline
...
\end{tabular}
\end{center}
\caption{Time averaged shear bulk shear stress for various volume fractions and aspect ratios. The time average is done between the first and second revolutions of the outer cylinder. As $\phi$ increases the bulk shear stress increases as expected. }\label{tab:viscosity}
\end{table}
