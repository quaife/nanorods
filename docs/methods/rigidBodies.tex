...
\end{center}
\todo[inline]{need normal in picture for the rigid bodies}
...
level set methods~\cite{Dou2007}, immersed boundary methods~\ref{}
\todo{inline}{Peskin, Chris Rycroft (Harvard)}, smoothed particle hydrodynamics~\cite{Polfer2016} or lattice Boltzmann
methods~\ref{} \todo[inline]{Georgia Tech guy from BIRS},
\todo[inline]{igor pivkin,  Li-shi luo}.  However, given that the fluid
...
The NCP solver is quite sensititive to the initial configuration.
...
step and restart with a smaller $\Delta t$. If, conversely, the NCP
converges in a small number of LCP iterations, we increase $\Delta t$
slightly.
\todo[inline]{put in numbers.  How much do we increase/decrease $\Delta
t$ by?}
...
horizon.  However, the local explicit time stepping results in a
contact, so the collision algorithm is required.  
\todo[inline]{If we shrink the time step size, does the local implicit
method reach the time horizon?  If so, for what $\Delta t$?  If not, how
small of $\Delta t$ have you tried?}
...
results are qualitatively similar to those reported in~\cite{Lu2017}.
\todo[inline]{Need more discussing results in
Figure~\ref{fig:shear_experiment}}
...
ratio, $\lambda$, 1, 3, and 6. The particles initial locations are
...
the alignment affects the material properties~\cite{} \todo[inline]{feel
free to motivate this entirely differently}.  One way to measure the
...
better with the flow \todo[inline]{need to explain why this happens and
include a reference}.
\begin{figure}[!h]
\begin{center}
...
better. This is because as in simple shear flow, the fibers still rotate
through all the orientations and align with the shear direction only on
average.  The 6:1 fibers rotate rotate through the angle perpendicular to
the shear direction more quickly than the 3:1 fibers and thus spend more
time approximately aligned with the shear direction. \todo[inline]{lots
in this cpation that isn't in the body}}
\end{figure} 


In shear flow particles align with the shear direction \emph{on
average}, however at any point in time they will in general not be
...
ellipse in an unbounded shear flow with shear rate $\dot{\gamma}$
...
\end{align*}
The time average order parameter is 1/2 for $\lambda= 3$ and 5/7$\approx
0.7143$ for $\lambda=6$ (independent of shear rate). Table
\ref{tab:order} shows the time and space averaged order parameter for
the Couette apparatus. We have arbitrarily decided to time average over
only the second revolution of the outer cylinder. We see that in all
...
contribute less to the bulk shear stress. The spikes in Figure 6
\todo[inline]{Need to use label and ref (not Figure 6)} occur when
