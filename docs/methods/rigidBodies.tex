...
is in~\cite{Lu2017}.  To apply a Lagrange multiplier method, the
governing equations~\eqref{eqn:modelEquations} are written as a
constrained optimization problem by using the variational form of the
Stokes equations
\begin{align}
  \min \int_{\Omega} \nabla\uu:\nabla\uu~\text{d}\Omega,
...
to quantify collision volumes.  By computing the STIV, rather than
...
boundary integrals~\cite{Pozrikidis1992} \textcolor{red}{(Section 2.5)}. 
...
the net force is zero \textcolor{red}{ and Newton's third law is not violated}.  However, if more than two bodies are in
...
torque on that body is scaled by the same amount. \textcolor{red}{This constraint is applied at every NCP iteration}.
...
with rotation matrices corresponding to the rotation velocity $\omega$. \textcolor{red}{This simple preconditioner significantly reduces the number of GMRES iterations needed to reach a given tolerance. For example, without preconditioning, the dense suspension in Section \ref{sec:taylor_green} requires approximately 100 GMRES iterations per time step. By applying the preconditioner, we can reduce the number of iterations to around 50 per time step.}
...
\begin{itemize}
  ...
  integrator proposed by Lu et al.~\cite{Lu2017} results in the STIV algorithm \textcolor{red}{
  creating a non-converging sequnce of LCPs}, even with $\Delta t = 10^{-8}$.
...
suspension, the bodies come into contact much more frequently. 
...
sizes must be taken.  Even when we took time step sizes as small as
$10^{-8}$, the method was not able to successfully reach the time
horizon.  This exact behavior has also been observed for vesicle
suspensions~\cite{Quaife2014}.  \textcolor{red}{Using a globally implicit time stepping method without a contact constraint requires a time step size smaller than $10^{-2}$ to avoid collisions. The overall computational cost for this example is comparable if no collision algorithm is employed because without collision avoidance the particles get much closer (within the buffer distance) and consequently the number of GMRES iterations grows from a maximum of around 50 when the collision avoidance algorithm is employed to well over 100.} 

\textcolor{red}{The added computational cost is almost entirely in checking if the nonlinear complemenarity problem given in \cite{Lu2017}  has been satisfied. Each check requires solving \eqref{eqn:BIEformulation} with different forces and torques on particles in contact. Thus for each NCP iteration we are increasing the cost of the time step by the amount of time it takes to solve \eqref{eqn:BIEformulation}. For this example, the maxmimum number of NCP iterations for a single time step was nine, and the vast majority required only one or two. If we look up to the time horizon $T=30$, $\Delta t = 2.5\e{-2}$ required a total of 190 NCP iterations (for 1200 time steps), $\Delta t = 1.25\e{-2}$ required  290 NCP iterations  (2400 time steps), and $\Delta t=6.25\e{-3}$ required 559 NCP iterations (4800 time steps). Thus the computational cost of the collision algorithm is 16\% for $\Delta t = 2.5\e{-2}$, 12\% for $\Delta t = 1.25\e{-2}$, and 12\% for  $\Delta t=6.25\e{-3}$. As mentioned above however, imposing the buffer region puts an upper bound on the number of GMRES iterations needed to solve each time step, and thus removing the collision avoidance algorithm (even if it is not strictly necessary to avoid collision) would increase the base computational cost.  }

In the bottom right plot of
...
ratio, $\lambda$ of 1, 3, and 6.  In all the examples, $\nu < 1/\ell^2$,
so all the suspensions are in the dilute regime.  The bodies initial
locations are random, but non-overlapping
(Figure~\ref{fig:couette_setup}).  The flow is driven by rotating the
outer cylinder at a constant angular velocity while the inner cylinder
remains fixed. 
