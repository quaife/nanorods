...
\end{align*}
A low cost first-order time stepper can be justified on the grounds that we are
interested in statistical, rheological, and bulk quantities rather than
...
iterations, and the maximum stable time step size is reduced.  In contrast, if the
interaction between the bodies is discretized implicitly, then the
stiffness caused by the nearly-touching bodies is resolved and a larger stable
