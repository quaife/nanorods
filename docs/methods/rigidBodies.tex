...
horizon without requiring a repulsion force.  However, with the same $\Delta t$, the local explicit time stepping results in a
collision between the particles, so the collision algorithm is required to reach the time horizon.  
\todo[inline]{By halving the time step size the locally implicit solver can also reach the time horizon without any repulsion forces. Is it worth mentioning this?}
...
time approximately aligned with the shear direction. }
\end{figure} 
