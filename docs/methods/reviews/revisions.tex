...
\begin{itemize}
  \item \todo[inline]{Lukas: Check if globally implicit without STIV
    works for Taylor-Green

LB : Done, results summarized on page 12}

  \item \todo[inline]{Lukas: Breakdown of the time spent doing NCP
    vs.~everything else.

LB : Done, results on page 12}
\end{itemize}
...
\begin{itemize}
  \item \todo[inline]{Lukas: Can you investigate the effect of the
    block-diagonal preconditioner when rigid bodies are close.  Do the
    Taylor-Green example from the paper without preconditioning.

LB : Done, page 10}
  \item \todo[inline]{Bryan: Describe how preconditioner can be rotated
    at each time step, so it is more or less free to update unlike
    vesicles where the shape changes}
\end{itemize}
...
\begin{itemize}
  \item \todo[inline]{Lukas: STIV stalls in the taylor-green example

LB: Brief mention on page 11}
  \item \todo[inline]{Bryan: Stiffness issue}
  \item \todo[inline]{Lukas: Plot the density function using 32 points
    for two 2:1 elliptical bodies that are separated from each by
    $10^{-1}, 10^{-2} \ldots 10^{-5}$.  Bryan expects that the
    density function does not blow up near  the contact and the velocity
...
\begin{itemize}
  \item \todo[inline]{Bryan: More resolution is required for higher aspect
    ratios so that the geometry can be resoled.  We only used 32 points
    per body, so we are restricted to aspect ratios not much larger than
    6:1.}
\end{itemize}
...
appendix.}
\begin{itemize}
  \item \todo[inline]{Bryan: Remove this and give a reference
    (Pozrikidis)}
\end{itemize}
...
\begin{itemize}
  \item \todo[inline]{Lukas: Find the reference to this in Pozrikidis
...
\begin{itemize}
  \item \todo[inline]{Lukas: We do this, just need to clarify in text
