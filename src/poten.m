...
if tEqualS % sources == targets
  if nsou > 1
    if (strfind(char(kernel),'fmm'))
      ...
      for k = 1:nsou
        K = [(1:k-1) (k+1:nsou)];
        [~,farField(:,k)] = kernelDirect(geomSou,f,[],Xtar(:,k),K);
        ...
        for k2 = K
            ...
            if (numel(J) ~= 0)
                ...
                if ((numel(J) + numel(fup)) >= 512 && numel(J) > 32)
                    [~,potTar] = kernel(geomUp,fup,[],[Xtar(J,k2);Xtar(J+Ntar,k2)],k1);
                else
                    [~,potTar] = kernelDirect(geomUp,fup,[],[Xtar(J,k2);Xtar(J+Ntar,k2)],k1);
                end
