...
stress_exact = @(x,y) 0.5*(grad_exact(x,y) + grad_exact(x,y)') - p_exact(x,y)*eye(2);

x = 1;
y = 1.5;

s_given = stress_exact(x,y);
p_given = p_exact(x,y);

for i = 1:length(N)
    prams.N = N(i); % points per body
    ...
    pressure(i) = pp.evaluatePressure(1, [x;y], 'fibers');
    [stress1(:,i), stress2(:,i)] = pp.evaluateStress(1, [x;y], 'fibers');
end

s_approx = [stress1(:,end),stress2(:,end)];
f = s_approx./s_given;