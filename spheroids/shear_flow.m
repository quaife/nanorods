...
a = 0.5;
b = 1;
particle2.x = a*cos(theta)+2;
particle2.y = b*sin(theta)+2;

particle2.jac = sqrt(a^2*sin(theta).^2 + b^2*cos(theta).^2);
particle2.n_x = b*cos(theta)./particle2.jac;
particle2.n_y = a*sin(theta)./particle2.jac;
cur_n = a*b;
cur_d = (a^2*sin(theta).^2 + b^2*cos(theta).^2).^(3/2);
particle2.cur = -cur_n./cur_d;
particle2.c = [2,2];
particle2.N = N;
particle2.tau_x = -a*sin(theta)./particle1.jac;
particle2.tau_y = b*cos(theta)./particle1.jac;
...
for i = 1:size(X,1)
    for j = 1:size(X,2)
        ...
        if (X(i,j)^2 + Y(i,j)^2 > 1.1 && (X(i,j)-2)^2 + (Y(i,j)-2)^2 >1.1)
            u(i,j,:) =  u_inf_handle(X(i,j),Y(i,j)) + evaluate_stokes_DLP_2_particles(X(i,j), Y(i,j), ...
